...
### 1. Client App Navigation (`lib/main.dart` ~800 lines)
```dart
MyApp (MaterialApp)
└── CategoryGallery (StatefulWidget) — Home page
    ├── SubcategoriesPage (StatefulWidget) — Subcategory selection
    │   └── ProductsPage (StatefulWidget) — Product grid with prices
...
2. **Subcategories** → Shows subcategories from nested `categories/{primary}/subcategories/` (24 documents across 8 primaries)
3. **Products** → Grid of products with images + prices (flat `products/` collection)
   - "Añadir" button adds to cart
   - Shows quantity controls (+/-) with trash icon when in cart
4. **Cart** → View cart, adjust quantities, send to WhatsApp
5. **Fullscreen** → Tap-to-zoom image view
...
- **Bulk Pricing:** Automatic discounts for LAT-2030 and LAT-1530
  - 1 unit: Q35
  - 2 units: Q30 each (Q60 total)
  - 5+ units: Q25 each
...
### 3. Stock Filtering
Currently, the client shows all products where `isActive: true`, regardless of stock levels. Future inventory management will filter based on `stockWarehouse + stockStore > 0`.

### 4. Firebase Hosting Build Path
`firebase.json` expects `build/web`. Common error: deploying without rebuilding.
```bash
# WRONG: firebase deploy (deploys old build)
# RIGHT:
flutter build web --release
firebase deploy
```
